...
// and a name no access point holds answers 404. The bool reports what that 404 cannot, because a
...
// whole answer because the RPC's user-name leaf states no cardinality. The match is on equality
// because most records carry the leaf empty, so an empty argument would select nearly every
// client, and the caller refuses one before this is reached.
