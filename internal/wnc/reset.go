...
// the session and not the access point. The name arm goes on the wire, so no address the
...
// that the instruction was accepted and nothing more. The reset-ap leaf is absent from the payload
